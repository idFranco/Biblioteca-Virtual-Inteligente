...
Whenever you need to create a new branch for a User Story, you MUST follow this exact sequence without skipping:
...
6. Create on Remote: Use the GitHub MCP tool `create-branch` (forbidden to use local `git branch` or `git checkout -b`).
7. Checkout locally: Run `git fetch origin` followed by `git checkout <new_branch_name>`.

- **Interactive Branch Names:** When a new User Story is being planned (during the `plan-user-story` phase), the agent is strictly forbidden from auto-generating or creating the Git branch autonomously.
...
3. This guarantees that side-effect writes from platform MCP tools (lifecycle state files, audit logs, indexing caches, etc.) are captured, regardless of which tool produced them.
...
If GitHub reports the branch is not mergeable, STOP and report it to the user — conflict resolution is a human action performed on GitHub, not an autonomous local git operation.