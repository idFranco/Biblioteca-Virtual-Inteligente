...
3. This guarantees that side-effect writes from any MCP tool (lifecycle state files, audit logs, indexing blocks, etc.) are captured, regardless of which tool produced them.
...
If GitHub reports the branch is not mergeable, HALT and report the conflict to the user — conflict resolution is a human action performed on GitHub, not an autonomous local git operation.
