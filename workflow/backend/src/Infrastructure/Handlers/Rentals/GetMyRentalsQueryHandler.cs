...
public sealed class GetMyRentalsQueryHandler : IQueryHandler<GetMyRentalsQuery, PagedResult<RentalResponse>>
{
    private readonly BibliotecaDbContext _context;

    public GetMyRentalsQueryHandler(BibliotecaDbContext context)
    {
        _context = context;
    }

    public async Task<PagedResult<RentalResponse>> HandleAsync(
        GetMyRentalsQuery query,
        CancellationToken cancellationToken = default)
    {
        ...
        var responseItems = items
            .Select(x => RentalMapper.ToResponse(x.Rental, x.BookTitle, x.UserEmail))
            .ToList();
