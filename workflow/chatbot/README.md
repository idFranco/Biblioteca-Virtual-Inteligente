...
- `smalltalk_prompt.txt`: respuesta breve/cortés para intención `other` (saludos/despedidas/agradecimientos); prohibe recomendar libros o inventar datos (US-020). **US-026:** distingue explícitamente **saludo** (saludar de vuelta, presentarse y **mantener el hilo abierto** con una pregunta/invitación a continuar — nunca despedirse) de **despedida/agradecimiento** (cierre cortés invite a volver), para que un "hola" no cierre la conversación. **US-028:** prohibe el **saludo duplicado** (no saludar de nuevo cuando el historial ya contiene intercambios previos) y las **despedidas prematuras** (no usar frases de cierre a menos que el usuario se despida); el historial conversacional se inyecta en el prompt para que el LLM lo tenga en cuenta.
- `guide_prompt.txt`: guía conversacional para lectores novatos (intención `guidance`, US-021); prohibe inventar títulos, referencia solo libros listados del catálogo, tono de bibliotecario, sugiere géneros accesibles. **US-028:** prohibe cerrar con frases de despedida (mantiene el hilo abierto).
